<div class="mt-1 relative" x-data="{open: @entangle('open')}" @click.away="open = false">
    <button
        type="button"
        @click="open = !open"
        class="relative h-12 w-full bg-gray-50 focus:bg-white transition duration-300 border border-gray-300 rounded-xl shadow-sm pl-5 pr-15 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
    ...
    <ul
        ...
        class="absolute mt-1 w-full bg-white shadow-lg max-h-56 rounded-xl py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm"
        tabindex="-1"
        role="listbox"
        aria-labelledby="listbox-label"
    ...
    @if (! empty($selected) && isset($selected[$trackBy]))
