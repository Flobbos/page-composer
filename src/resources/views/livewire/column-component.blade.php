<div x-data="{ showConfirm: false, showColumnSettings: false }" class="relative flex flex-col items-start justify-center px-3 pt-5 pb-3 bg-white border border-transparent rounded-lg hover:border-gray-100 group/column">
    ...
    <button @click.prevent="showColumnSettings = !showColumnSettings"
    ...
    <div x-show="showColumnSettings" x-cloak
        class="absolute z-50 flex flex-col @if ($column['column_size'] <= 6) w-full @else w-1/2 @endif p-5 text-xs bg-white border border-gray-200 rounded-lg shadow-lg top-4 right-7
            space-y-2">
        ...
        <div class="flex flex-col w-full">
            ...
            <div class="flex justify-end space-x-5">
                <button class="px-2 py-1 text-xs text-white bg-red-600 rounded hover:bg-red-400 focus:outline-none" @click.prevent="showColumnSettings = false">{{ __('Cancel') }}</button>
    ...
    <div class="flex flex-col w-full">
        @foreach ($this->sortedElements as $key => $item)
            <div>
                @livewire('page-composer-elements.' . $item['component'], ['data' => $item, 'itemKey' => $key, 'sorting' => $this->getElementPositionArray($key), 'previewMode' => $previewMode, 'target' => $source], key($source . '-item-' . $key . '-sort-' . Arr::get($item, 'sorting', 0) . '-' . ($previewMode ? 'preview' : 'schema')))
            </div>
        @endforeach
    </div>
    <div class="flex justify-center w-full">
        <button @click.prevent="$dispatch('show-element-selector',{id: '{{ $this->id() }}'})" @class([
            'flex items-center justify-center w-full px-2 py-1 space-x-1 text-sm text-green-400 transition bg-green-100 rounded-full group-hover/column:visible focus:outline-none hover:bg-green-400 hover:text-green-100',
            'invisible' => !empty($this->sortedElements),
        ])>
