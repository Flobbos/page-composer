    ...
    <x-slot name="content">
        ...
        <div class="grid grid-cols-6 gap-6 mb-6">
            ...
            <div class="col-span-6 sm:col-span-3">
                <label for="page_title" class="block mb-2 text-xs font-medium text-gray-700">{{ __('Titel (intern)') }}</label>
        ...
        <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-3">
                <label for="category" class="block mb-2 text-xs font-medium text-gray-700">{{ __('Kategorie') }}</label>
                <livewire:select-input name="category_id" placeholder="{{ __('Kategorie auswählen') }}" :options="$categories" :selected="$pageCategory" labelBy="name" :key="uniqid()" />
            </div>
            <div class="col-span-6 sm:col-span-3">
                <label for="tags" class="block mb-2 text-xs font-medium text-gray-700">{{ __('Tags') }}</label>
                <livewire:multi-select-input name="tags" placeholder="{{ __('Tags auswählen') }}" :options="$tags" :key="uniqid()" labelBy="name" eventName="tagsUpdated" :selected="$pageTags" />
            </div>
